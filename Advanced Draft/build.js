...
const options = {
	...
	workers: "\n\"use strict\";\n" +
		...
		`${fs.readFileSync("ource/sheetworkers.js", "utf8").trim()}\n`
};
...
sass.render({
	...
	if (!error) {
		...
		fs.writeFile("lifeAfter.css", cssOutput, printOutput);
	} else {
...
// Build HTML
const htmlOutput = pug.renderFile("source/lifeAfter.pug", options).trim().replace(/\n+/g, "\n");
fs.writeFile("lifeAfter.html", `${htmlOutput}\n`, printOutput);
